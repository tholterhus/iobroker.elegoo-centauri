...
class MonitoringHandler {
    constructor(adapter) {
        ...
        this.ws = null;
        this.pollTimer = this.adapter.config.pollInterval;;
        this.reconnectTimer = this.adapter.config.reconnectInterval;
    }

    connect() {
//        const url = `ws://${this.adapter.config.printerIp}:${this.adapter.config.wsPort}`;
        const url = `ws://${this.adapter.config.printerIp}/websocket`;
        this.adapter.log.info(`Connecting to printer at ${url}`);
        this.ws = new WebSocket(url, {
            headers: {
                'User-Agent': 'ioBroker-Elegoo-Centauri/0.0.1'
            },
            timeout: 10000
        });
        ...
        this.ws.on('message', (data) => {
            this.adapter.log.info('Received message.');
    ...
    startPolling() {
        this.adapter.log.info(`start polling`);
        ...
        const poll = () => {
            ...
            this.pollTimer = setTimeout(poll, this.adapter.config.pollInterval);
        };
        poll();
        this.adapter.log.info(`polling done. Timer set to ${this.adapter.config.pollInterval}s.`);
    }
