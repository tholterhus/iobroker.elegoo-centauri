...
class MonitoringHandler {
    ...
    connect() {
        const url = `ws://${this.adapter.config.printerIp}:${this.adapter.config.wsPort}/`;
    ...
    async _handleMessage(message) {
        try {
            ...
            if (payload.command === 'GET_STATUS_RSP' && payload.data) {
                ...
                const statusKey = STATUS_CODES[statusCode] |
...
| 'UNKNOWN';
                ...
                await this.adapter.setStateAsync('status.printProgress', data.progress |

| 0, true);
                await this.adapter.setStateAsync('status.currentLayer', data.current_layer |

| 0, true);
                await this.adapter.setStateAsync('status.totalLayers', data.total_layers |

| 0, true);

                // Update Timings
                const elapsed = data.print_time_secs |

| 0;
                const remaining = data.remaining_time_secs |
...
| 0;
                ...
                // Update Temperatures
                if (data.temps) {
                    await this.adapter.setStateAsync('temperatures.nozzle', data.temps.nozzle?.current |

| 0, true);
                    await this.adapter.setStateAsync('temperatures.nozzleTarget', data.temps.nozzle?.target |

| 0, true);
                    await this.adapter.setStateAsync('temperatures.bed', data.temps.bed?.current |

| 0, true);
                    await this.adapter.setStateAsync('temperatures.bedTarget', data.temps.bed?.target |

| 0, true);
                    await this.adapter.setStateAsync('temperatures.enclosure', data.temps.enclosure |
...
| 0, true);
                ...
                // Update Fans
                if (data.fans) {
                    await this.adapter.setStateAsync('fans.model', data.fans.model |

| 0, true);
                    await this.adapter.setStateAsync('fans.auxiliary', data.fans.aux |

| 0, true);
                    await this.adapter.setStateAsync('fans.enclosure', data.fans.enclosure |
